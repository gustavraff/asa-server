...
- Context rollover: if the task changes substantially, context has already been compacted, or work passes roughly 25 tool calls, stop before starting another large phase. Run `AI-Usage-Audit.ps1 -CreateHandoff -CurrentTask "..." -NextStep "..."`, tell Gustav the handoff is ready, and request one manual New Thread click. Codex must not claim it can create or navigate to a new thread unless the product exposes that action in the current session.
- In a fresh thread, read `AI-THREAD-HANDOFF.md` when present, then verify Git state. Treat the handoff as a concise pointer, not as proof that live server state is unchanged.
